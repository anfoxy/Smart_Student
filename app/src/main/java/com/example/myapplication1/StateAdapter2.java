   ...
   //    this.onClickListener = OnClickListener;
    ...
    public void onBindViewHolder(StateAdapter2.ViewHolder holder, int position) {
        ...
        if(state3.getInd()==-1){
            ...
            holder.planView.setText("Выучено " + state3.getKol() + " из " + MainActivity2.ArrPlan.get(state3.getInPredmeta()).plan_to_day.get(state3.getInd()).size_of_quetion);
            if (MainActivity2.ArrPlan.get(state3.getInPredmeta()).plan_to_day.get(state3.getInd()).size_of_quetion <= MainActivity2.ArrPlan.get(state3.getInPredmeta()).todaylearned) {
                holder.Otvet.setImageResource(R.drawable.ic_baseline_replay_24);
                holder.planView.setText("Выучено " + MainActivity2.ArrPlan.get(state3.getInPredmeta()).plan_to_day.get(state3.getInd()).size_of_quetion + " из " + MainActivity2.ArrPlan.get(state3.getInPredmeta()).plan_to_day.get(state3.getInd()).size_of_quetion);
            }
        }
        ...
        holder.Otvet.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                }else if(MainActivity2.ArrPlan.get(state3.getInPredmeta()).plan_to_day.get(state3.getInd()).size_of_quetion <= MainActivity2.ArrPlan.get(state3.getInPredmeta()).todaylearned){
                ...
                Navigation.findNavController(v).navigate(R.id.action_navigation_home_to_navigation_question);
            }
       });
