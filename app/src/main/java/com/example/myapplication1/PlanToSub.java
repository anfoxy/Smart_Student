...
public class PlanToSub {
    public  Subject sub; //предмет
    public  ArrayList<PlanToDay> plan_to_day; //планы на все дни
    public  int todaylearned; //сколько выучили именно сегодня
    public Date_simple today;
    ...
    public  void open_prog(){
        ...
        if(today.before(av)) { //тут проверяется наступил ли новый день
            ...
            if (a!=-1){
                ...
                MainActivity2.myDBManager.update_today_ost_que(sub.name_of_sub,plan_to_day.get(a).date,todaylearned);
            }
            todaylearned=0;
            MainActivity2.myDBManager.update_todaylearning(sub.name_of_sub,0);
            today=av;
            MainActivity2.myDBManager.update_tDATE(sub.name_of_sub,av);
            ArrayList<PlanToDay> p = new  ArrayList<PlanToDay>(plan_to_day);
            do_plan();
            MainActivity2.myDBManager.update_Allplantoday(sub.name_of_sub,p,plan_to_day);
        }
    }
    ...
    public  int size_today_day_of_exams_plan(){
        ...
        int res=0;
        for(int i=0; i<plan_to_day.size();i++){
            if((plan_to_day.get(i).date.before(date_of_exams) &&(a.before(plan_to_day.get(i).date)||a.equally(plan_to_day.get(i).date))))
                res++;
        }
        return res;
    }
    ...
    public Subject getS(){return sub;}
