...
public class MyDBManager {
    ...
    public  void get(PlanToSub pl){
        ...
        for(int i=0;i<pl.sub.getQuestion().size();i++){
            insert_TABLE_QUE(pl.sub.name_of_sub,pl.sub.getQuestion().get(i));
        }
    ...
    public void update_plantoday(String name,PlanToDay p, PlanToDay p1) {
        ...
        ContentValues cv = new ContentValues();
        cv.put(MyConstants.TODAY, p.date.day);
        cv.put(MyConstants.TOMONTH, p.date.month);
        cv.put(MyConstants.TOYEAR, p.date.year);
        cv.put(MyConstants.TODAY_OST_QUE, p.size_of_quetion);
        ...
        do {
            ...
            int tomonth = cursor.getInt(cursor.getColumnIndex(MyConstants.TOMONTH));
            int toyear = cursor.getInt(cursor.getColumnIndex(MyConstants.TOYEAR));
            if (p1.date.day == today && p1.date.month == tomonth && p1.date.year == toyear && name.equals(n)) {
                int ind = cursor.getInt(cursor.getColumnIndex(MyConstants._ID));
    ...
    public void insert_TABLE_SUBJ(PlanToSub pl) {
        ...
        cv.put(MyConstants.SUBJECT,pl.sub.name_of_sub);
        cv.put(MyConstants.DAY, pl.getDay());
        cv.put(MyConstants.MONTH, pl.getMonth());
        cv.put(MyConstants.YEAR, pl.getYear());
        cv.put(MyConstants.TODAYLEARNED, pl.todaylearned);
        cv.put(MyConstants.TDAY, pl.today.day);
        cv.put(MyConstants.TMONTH, pl.today.month);
        cv.put(MyConstants.TYEAR, pl.today.year);

        db.delete(MyConstants.TABLE_NAME_SUBJECTS, MyConstants.SUBJECT + " = ?", new String[]{pl.sub.name_of_sub});
    ...
    public void insert_TABLE_TODAY(PlanToSub pl) {
        ContentValues cv = new ContentValues();
        for (int i = 0; i < pl.plan_to_day.size(); i++) {
            cv.put(MyConstants.SUBJECT, pl.sub.name_of_sub);
            cv.put(MyConstants.TODAY, pl.plan_to_day.get(i).date.day);
            cv.put(MyConstants.TOMONTH, pl.plan_to_day.get(i).date.month);
            cv.put(MyConstants.TOYEAR, pl.plan_to_day.get(i).date.year);
            cv.put(MyConstants.TODAY_OST_QUE, pl.plan_to_day.get(i).size_of_quetion);
    ...
    public void insert_TABLE_TODAY2(String name_of_sub, PlanToDay pl) {
        ContentValues cv = new ContentValues();
            ...
            cv.put(MyConstants.TODAY, pl.date.day);
            cv.put(MyConstants.TOMONTH, pl.date.month);
            cv.put(MyConstants.TOYEAR, pl.date.year);
            cv.put(MyConstants.TODAY_OST_QUE, pl.size_of_quetion);
            db.insert(MyConstants.TABLE_NAME_TODAY, null, cv);
    }
